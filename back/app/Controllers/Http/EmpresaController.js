    ...
    async criarEmpresa({ request, response }) {
        ...
        if (dataToCreate.uid === null) {
            ...
            retorno = {
                erro: {
                    codigo: 0,
                    msg: "UID vazio para cadastrar nova empresa",
                },
            };
        } else {
            ...
            const idUser = await Database.select("*")
                .table("funcionarios")
            ...
            if (idUser.length === 0) {
                ...
                retorno = {
                    erro: {
                        codigo: 22,
                        msg: "Funcionario nao encontrado no banco de dados",
                    },
                };
            } else {
                ...
                if (
                    dataToCreate.fantasia === null ||
                    dataToCreate.fantasia === undefined
                ) {
                    response?.status(400);
                    retorno = {
                        erro: {
                            codigo: 1,
                            msg: "Nome ou Fantasia não preenchido para criar empresa",
                        },
                    };
                } else {
    ...
    async listarEmpresas({ request, response }) {
        ...
        if (user.uid === "" || user.uid === undefined || user.uid === null) {
            ...
            retorno = {
                erro: {
                    codigo: 29,
                    msg: "Parametros invalidos para buscar empresas vinculadas ao funcionario",
                },
            };
        } else {
        ...
        if (
            ...
            retorno = {
                erro: {
                    codigo: 2,
                    msg:
                        "Parametros invalidos para buscar dados, parametro passado:" +
                        params.id,
                },
            };
        } else {
                ...
                retorno = {
                    erro: {
                        ...
                        msg:
                            "Empresa com ID:" +
                            params.id +
                            " Nao encontrada para buscar os dados",
                    },
                };
            } else {
                const user = request.only(["uid"]);
                const verificador = await Database.select("funcionario")
                    .table("funcionario_empresas")
                    .where("funcionario_uid", user.uid)
                    .where("empresa", params.id);
                if (verificador.length <= 0) {
                    response?.status(404);
                    retorno = {
                        erro: {
                            codigo: 35,
                            msg: "Funcionário não vinculado a empresa",
                        },
                    };
                } else {
    ...
    async alterarEmpresa({ params, request, response }) {
        ...
        if (
            ...
            retorno = {
                erro: {
                    codigo: 6,
                    msg: "Parametro ID inválido para alterar os dados da empresa",
                },
            };
        } else {
            ...
            if (empresa === null) {
                ...
                const atualizaEmpresa = request.only([
                    ...
                    "criador",
                    "uid",
                ]); //uid de quem ta alterando a empresa
                //verifica se o campo de "uid" foi preenchido
                if (
                    atualizaEmpresa.uid === null ||
                    atualizaEmpresa.uid === "" ||
                    atualizaEmpresa.uid === undefined
                ) {
                    response?.status(400);
                    retorno = {
                        erro: {
                            codigo: 8,
                            msg: "UID invalido para alterar dados da empresa",
                        },
                    };
                } else {
                    const idUser = await Database.select("id", "nome")
                        .table("funcionarios")
                        .where("uid", atualizaEmpresa.uid);
                    ...
                    if (idUser.length === 0) {
                        ...
                        if (funcionarioEmpresa.length === 0) {
                            ...
                            retorno = {
                                erro: {
                                    codigo: 25,
                                    msg: "Funcionario nao esta vinculado a esta empresa",
                                },
                            };
                        } else {
                            ...
                            if (funcionarioEmpresa[0].cargo !== "A") {
                                ...
                                retorno = {
                                    erro: {
                                        codigo: 24,
                                        msg: "Funcionario sem permissao para alterar os dados da empresa",
                                    },
                                };
                            } else {
                                if (
                                    atualizaEmpresa.fantasia === null ||
                                    atualizaEmpresa.fantasia === undefined
                                ) {
                                    response?.status(400);
                                    retorno = {
                                        erro: {
                                            codigo: 9,
                                            msg: "Nome ou Fantasia não preenchido para alterar dados da empresa",
                                        },
                                    };
                                } else {
                                    ...
                                    await logC.novoLog({
                                        request: {
                                            ...
                                            valornovo:
                                                JSON.stringify(atualizaEmpresa),
                                            funcionario:
                                    ...
                                    empresa.userm =
                                        funcionarioEmpresa[0].funcionario;
    ...
    async deletarEmpresa({ params, request, response }) {
        ...
        if (params.id === "" || params.id === null || params.id === undefined) {
            ...
            retorno = {
                erro: {
                    codigo: 4,
                    msg: "parametros preenchidos errados para deletar empresa",
                },
            };
        } else {
            const empresa = await Empresa.find(params.id);
            if (empresa == null) {
                ...
                if (
                    ...
                    retorno = {
                        erro: {
                            codigo: 17,
                            msg: "UID não preenchido para DELETAR empresa",
                        },
                    };
                } else {
                    ...
                    if (idUser.length === 0) {
                        ...
                        retorno = {
                            erro: {
                                codigo: 26,
                                msg: "Funcionario nao encontrado no banco de dados para DELETAR os dados da empresa",
                            },
                        };
                    } else {
                        //busca no banco de dados o funcionario vinculado a empresa
                        ...
                        if (funcionarioEmpresa.length === 0) {
                            ...
                            retorno = {
                                erro: {
                                    codigo: 27,
                                    msg: "Funcionario nao vinculado a empresa para DELETAR",
                                },
                            };
                        } else {
                            ...
                            if (funcionarioEmpresa[0].cargo !== "A") {
                                ...
                                retorno = {
                                    erro: {
                                        codigo: 28,
                                        msg: "Funcionario sem permissao para DELETAR empresa",
                                    },
                                };
                            } else {
                                const temp = empresa;
