...
class ClienteController {
                ...
                if(dadosRequest.fantasia=== null || dadosRequest.fantasia===undefined || dadosRequest.fantasia===''){
                    ...
                    if(userm?.length === 0){
                        ...
                        if(teste){
                            ...
                            response?.status(500)
                            retorno = {erro:{codigo:70,msg:"Algo deu errado em salvar cliente"}}
                        }
                    }
                }
            }
    ...
    async deletarCliente({params,request,response}){
        ...
        if(!cliente){
            ...
            if(!dadosRequest.uid){
                response?.status(400)
                retorno = {erro:{codigo:72,msg:"Não informada UID para excluir cliente"}}
            }else{
                const userm = await Database.select('*')
                .table('funcionario_empresas')
                .where('funcionario_uid',dadosRequest.uid)
                ...
                if(userm[0]?.empresa !== cliente.empresa){
                    ...
                    if(verificador){
                        response?.status(403)
                        retorno = {erro:{codigo:74,msg:"Não foi possivel excluir, cliente tem um ou mais protocolos vinculados"}}
                    }else{
                        await logC.novoLog({
                            request: {
                                operacao: "EXCLUIR",
