...
class ObservacaoController {
    async criarObservacao({request,response}){
        let retorno = ""
        const dadosRequest = request.only(['protocolo','observacao','uid','empresa'])
        const userc = await Database.select("*")
            ...
            if (userc.length === 0) {
                response?.status(404)
                retorno = {erro:{codigo:105,msg:'Usuário não encontrado ou não vinculado a empresa para observar protocolo'}}
            }else{
                ...
                if(protocolo.length ===0){
                    response?.status(404)
                    retorno = {erro:{codigo:106,msg:'Protocolo não encontrado para ser observado'}}
                }else{
                    if(protocolo[0].situacao === "C" || protocolo[0].situacao==="c"){
                        response?.status(403)
                        retorno = {erro:{codigo:107,msg:"Protocolo já concluido"}}
                    }else{
